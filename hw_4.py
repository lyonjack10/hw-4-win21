...
class Customer:
	...
	def order_food(self, server, truck, food_name, quantity):
		if not(server.know_truck(truck)):
			print("Sorry, I don't know that food truck. Please try a different one.")
...
class TestAllMethods(unittest.TestCase):
	...
	# Check to see that the server knows what different food trucks look like
	def test_adding_and_knowing_truck(self):
		s3 = Server("Felix", service_fee = 7, food_trucks = [self.t1, self.t2])
		self.assertTrue(s3.know_truck(self.t1))
		self.assertFalse(s3.know_truck(self.t3))
		s3.add_truck(self.t3)
		self.assertTrue(s3.know_truck(self.t3))
